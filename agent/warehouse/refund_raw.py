"""Validate immutable refund pages before exposing exact-response raw rows.

No Shopify calls or GCS mutations. Publication and checkpoint changes are the
caller's responsibility. Rows are HTTP response pages, not reconstructed refunds.
"""
from datetime import datetime

from .refund_capture import CaptureError, RefundCapture, decode, digest, encoded
...
def prepare_refund_raw(*, bucket, domain, api_version, shop_gid, extraction_id,
    ...
    capture = RefundCapture(bucket=bucket, domain=domain, token="", api_version=api_version,
    ...
    seal = capture.collect()  # Exhaustively revalidate saved request/cursor/owner chains.
    ...
    files.append(dict(uri=f"gs://{bucket.name}/{seal_blob.name}", generation=str(seal_blob.generation),
                      sha256=digest(seal_bytes), role="completion_seal"))
    ...
    def records():
        for page in seal["pages"]:
            ...
            yield dict(shop_key=shop_gid, extraction_id=extraction_id, file_id=page["generation"],
                record_index=1, query_sha256=seal["binding"]["query_sha256"], request_sha256=request_sha,
                api_version=api_version, ingested_at=ingested_at.isoformat(), record_sha256=page["sha256"],
    ...
    return dict(records=records(), files=files, counts=seal["counts"],
                ...
                request_sha256=request_sha, started_at=min(timestamps), completed_at=max(timestamps))
