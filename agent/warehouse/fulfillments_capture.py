"""Pinned, independently paginated capture for Shopify fulfillments.

Children of orders, following the returns pattern: orders own the root cursor
and each order's fulfillments are re-read through an owner-scoped page.  A
fulfillment must have exactly one owning order within the extraction.
``Order.fulfillments`` is read as a plain list; a connection-shaped response
fails closed because the list interpretation cannot be verified offline.
"""
...
from .fulfillments_queries import compile_fulfillment_queries


class FulfillmentsCapture:
    def __init__(self, *, bucket, domain, token, api_version, shop_gid, extraction_id,
        ...
        self.operations = dict(zip(("orders", "fulfillments"),
                                   compile_fulfillment_queries(query_source).documents()))
        self.binding = {"format_version": 1, "stream": "fulfillments", "domain": domain,
        ...
        key = digest(encoded([shop_gid, extraction_id, "fulfillments"]))
        self.prefix = f"pages/v1/fulfillments/{key}"
    ...
    def _bind(self):
        ...
        try:
            ...
            if decode(blob.download_as_bytes(if_generation_match=int(blob.generation))) != self.binding:
                raise CaptureError("Extraction identity is already bound to another fulfillments plan")
    ...
    def _http(self, document, variables):
        ...
        try:
            with requests.Session() as session:
                ...
                with session.post(f"https://{self.domain}/admin/api/{self.api_version}/graphql.json",
                    ...
                    if response.status_code != 200 or response.headers.get("X-Shopify-API-Version") != self.api_version:
                        raise CaptureError("Shopify fulfillments page request failed or API version mismatched")
                    body = bytearray()
                    for chunk in response.iter_content(chunk_size=65536):
                        if time.monotonic() >= self.deadline:
                            raise CaptureError("Fulfillments capture deadline reached")
                        body.extend(chunk)
            ...
            raise CaptureError("Shopify fulfillments transport failed; response details suppressed") from None

    def fetch(self, operation, variables):
        if self._finished or time.monotonic() >= self.deadline or len(self.pages) >= self.max_pages:
            raise CaptureError("Fulfillments capture deadline, page limit, or sealed capture reached")
        ...
        if existing is None or existing.generation is None or existing.size > 2 * 1024 * 1024:
            raise CaptureError("Missing or oversized captured fulfillments page")
        ...
        if metadata.get("response_sha256") != digest(body) or metadata.get("request_sha256") != request_hash:
            raise CaptureError("Captured fulfillments page checksum or identity mismatch")
        try:
            if datetime.fromisoformat(metadata["captured_at"]).utcoffset() is None:
                raise ValueError()
        except (ValueError, TypeError, KeyError):
            raise CaptureError("Captured fulfillments page timestamp is missing or invalid") from None
        self.bytes += len(body)
        if self.bytes > self.max_bytes:
            raise CaptureError("Total fulfillments capture size limit reached")
        data = decode(body)
        if data.get("errors") or not isinstance(data.get("data"), dict):
            raise CaptureError("Captured fulfillments GraphQL response is incomplete or failed")
    ...
    def walk(self, operation, owner=None):
        if operation == "fulfillments":
            yield from self._walk_fulfillments(owner)
            return
        after, cursors, identifiers = None, set(), set()
        while True:
            variables = {"first": self.page_size, "after": after, "query": self.search_filter}
    ...
    def _walk_fulfillments(self, owner):
        order_id = gid(owner, "Order")
        variables = {"id": order_id}
        data = self.fetch("fulfillments", variables)
        node = data.get("node")
        if not isinstance(node, dict) or node.get("id") != order_id:
            raise CaptureError("Fulfillment page owner mismatch")
        fulfillments = node.get("fulfillments")
        if not isinstance(fulfillments, list):
            raise CaptureError("Fulfillments page does not contain a fulfillment list")
        identifiers = set()
        for item in fulfillments:
            if not isinstance(item, dict) or not isinstance(item.get("id"), str):
                raise CaptureError("Invalid fulfillment identity")
            if item["id"] in identifiers:
                raise CaptureError("Duplicate fulfillment within an order page")
            identifiers.add(item["id"])
            yield item

    def collect(self):
        counts = {"orders": 0, "fulfillments": 0}
        # A Fulfillment must have exactly one owning Order within this
        # extraction; ``walk`` scopes its duplicate detector to one page, so
        # enforce the cross-order invariant before writing the completion seal.
        fulfillment_owners = {}
        for order in self.walk("orders"):
            order_id = gid(order.get("id"), "Order")
            counts["orders"] += 1
            for fulfillment in self.walk("fulfillments", order_id):
                fulfillment_id = gid(fulfillment.get("id"), "Fulfillment")
                previous_owner = fulfillment_owners.setdefault(fulfillment_id, order_id)
                if previous_owner != order_id:
                    raise CaptureError("Fulfillment is linked to multiple orders in one extraction")
        ...
        if self.read_only:
            ...
            if existing is None or existing.download_as_bytes(if_generation_match=int(existing.generation)) != content:
                raise CaptureError("Missing or conflicting fulfillments completion seal")
        else:
            try:
                ...
                if blob.download_as_bytes(if_generation_match=int(blob.generation)) != content:
                    raise CaptureError("Conflicting completed fulfillments capture")
        self._finished = True
        return seal
