"""Pinned, independently paginated capture for Shopify inventory streams.

Inventory items own a root cursor; locations own a second root cursor whose
nodes are re-read through owner-scoped inventory-level pages.  An inventory
level must have exactly one owning location within the extraction.
"""
...
from .inventory_queries import compile_inventory_queries


class InventoryCapture:
    def __init__(self, *, bucket, domain, token, api_version, shop_gid, extraction_id,
                 items_source, levels_source, search_filter, page_size=50,
        ...
        plan = compile_inventory_queries(items_source, levels_source)
        self.operations = dict(zip(("inventoryItems", "locations", "inventoryLevels"),
                                   plan.documents()))
        self.binding = {
            "format_version": 1, "stream": "inventory", "domain": domain, "shop_gid": shop_gid,
            "api_version": api_version, "extraction_id": extraction_id,
            "query_sha256": {"inventoryItems": digest(items_source.encode()),
                             "inventoryLevels": digest(levels_source.encode())},
            "plan_sha256": digest(encoded(self.operations)),
            "scope_sha256": digest(encoded({"query": search_filter, "first": page_size})),
        }
        key = digest(encoded([shop_gid, extraction_id, "inventory"]))
        self.prefix = f"pages/v1/inventory/{key}"
    ...
    def _bind(self):
        ...
        try:
            ...
            if decode(blob.download_as_bytes(if_generation_match=int(blob.generation))) != self.binding:
                raise CaptureError("Extraction identity is already bound to another inventory plan")
    ...
    def _http(self, document, variables):
        ...
        try:
            with requests.Session() as session:
                ...
                with session.post(f"https://{self.domain}/admin/api/{self.api_version}/graphql.json",
                    ...
                    if response.status_code != 200 or response.headers.get("X-Shopify-API-Version") != self.api_version:
                        raise CaptureError("Shopify inventory page request failed or API version mismatched")
                    body = bytearray()
                    for chunk in response.iter_content(chunk_size=65536):
                        if time.monotonic() >= self.deadline:
                            raise CaptureError("Inventory capture deadline reached")
                        body.extend(chunk)
            ...
            raise CaptureError("Shopify inventory transport failed; response details suppressed") from None

    def fetch(self, operation, variables):
        if operation not in self.operations:
            raise CaptureError("Unknown inventory operation")
        if self._finished or time.monotonic() >= self.deadline or len(self.pages) >= self.max_pages:
            raise CaptureError("Inventory capture deadline, page limit, or sealed capture reached")
        request_hash = digest(encoded({"query": self.operations[operation], "variables": variables}))
        if request_hash in self._request_keys:
            raise CaptureError("Duplicate inventory page request within traversal")
        name = f"{self.prefix}/{request_hash}.json"
        existing = self.bucket.get_blob(name)
        if existing is None:
            if self.read_only:
                raise CaptureError("Missing inventory page in read-only capture")
        ...
        if existing is None or existing.generation is None or existing.size > 2 * 1024 * 1024:
            raise CaptureError("Missing or oversized captured inventory page")
        ...
        if metadata.get("response_sha256") != digest(body) or metadata.get("request_sha256") != request_hash:
            raise CaptureError("Captured inventory page checksum or identity mismatch")
        try:
            if datetime.fromisoformat(metadata["captured_at"]).utcoffset() is None:
                raise ValueError()
        except (ValueError, TypeError, KeyError):
            raise CaptureError("Captured inventory page timestamp is missing or invalid") from None
        self.bytes += len(body)
        if self.bytes > self.max_bytes:
            raise CaptureError("Total inventory capture size limit reached")
        data = decode(body)
        if data.get("errors") or not isinstance(data.get("data"), dict):
            raise CaptureError("Captured inventory GraphQL response is incomplete or failed")
    ...
    def _walk_root(self, operation, filtered):
        ...
        while True:
            ...
            if not isinstance(connection, dict):
                raise CaptureError("Missing requested inventory connection")
            ...
            if (not isinstance(info, dict) or type(info.get("hasNextPage")) is not bool
                ...
                raise CaptureError("Invalid inventory pagination response")
            next_cursor = info.get("endCursor")
            if info["hasNextPage"] and not edges:
                raise CaptureError("Nonadvancing inventory pagination cursor")
            ...
            for edge in edges:
                ...
                if not isinstance(node, dict) or not isinstance(node.get("id"), str):
                    raise CaptureError("Invalid inventory object identity")
                if node["id"] in identifiers:
                    raise CaptureError("Duplicate inventory object across pages")
    ...
    def walk(self, operation, owner=None):
        if operation == "inventoryLevels":
            yield from self._walk_levels(owner)
        elif operation == "inventoryItems":
            yield from self._walk_root(operation, filtered=True)
        else:
            yield from self._walk_root(operation, filtered=False)

    def _walk_levels(self, owner):
        after, cursors, identifiers = None, set(), set()
        location_id = gid(owner, "Location")
        while True:
            variables = {"first": self.page_size, "after": after, "id": location_id}
            data = self.fetch("inventoryLevels", variables)
            node = data.get("node")
            if not isinstance(node, dict) or node.get("id") != location_id:
                raise CaptureError("Inventory level page owner mismatch")
            connection = node.get("inventoryLevels")
            if not isinstance(connection, dict):
                raise CaptureError("Missing requested inventory connection")
            info, edges = connection.get("pageInfo"), connection.get("edges")
            if (not isinstance(info, dict) or type(info.get("hasNextPage")) is not bool
            ...
            for edge in edges:
                ...
                if not isinstance(item, dict) or not isinstance(item.get("id"), str):
                    raise CaptureError("Invalid inventory level identity")
                if item["id"] in identifiers:
                    raise CaptureError("Duplicate inventory level across pages")
    ...
    def collect(self):
        counts = {"inventoryItems": 0, "locations": 0, "inventoryLevels": 0}
        for _ in self.walk("inventoryItems"):
            counts["inventoryItems"] += 1
        # An InventoryLevel must have exactly one owning Location within this
        # extraction; the per-page duplicate detector cannot see across
        # locations, so enforce the cross-owner invariant here.
        level_owners = {}
        for location in self.walk("locations"):
            location_id = gid(location.get("id"), "Location")
            counts["locations"] += 1
            for level in self.walk("inventoryLevels", location_id):
                level_id = level["id"]
                previous_owner = level_owners.setdefault(level_id, location_id)
                if previous_owner != location_id:
                    raise CaptureError("Inventory level is linked to multiple locations in one extraction")
        ...
        if self.read_only:
            ...
            if existing is None or existing.download_as_bytes(if_generation_match=int(existing.generation)) != content:
                raise CaptureError("Missing or conflicting inventory completion seal")
        else:
            try:
                ...
                if blob.download_as_bytes(if_generation_match=int(blob.generation)) != content:
                    raise CaptureError("Conflicting completed inventory capture")
        self._finished = True
        return seal
