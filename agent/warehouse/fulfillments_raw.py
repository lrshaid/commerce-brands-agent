"""Validate sealed fulfillments pages before exposing exact-response raw rows.

No Shopify calls or GCS mutations. Rows are original HTTP response pages, never
flattened or reassembled business objects.
"""
from datetime import datetime

from .fulfillments_capture import FulfillmentsCapture
from .refund_capture import CaptureError, decode, digest, encoded


def prepare_fulfillments_raw(*, bucket, domain, api_version, shop_gid, extraction_id,
    ...
    def records():
        for page in seal["pages"]:
            ...
            if blob is None or str(blob.generation) != page["generation"]:
                raise CaptureError("Captured fulfillments page generation changed")
            body = blob.download_as_bytes(if_generation_match=int(page["generation"]))
            if digest(body) != page["sha256"]:
                raise CaptureError("Captured fulfillments page checksum changed")
            decode(body)
