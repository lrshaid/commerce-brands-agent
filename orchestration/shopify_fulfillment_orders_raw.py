"""Publish a sealed bulk export as fulfillment-order and line-item streams."""
...
def shopify_fulfillment_orders_raw(context: dg.AssetExecutionContext, config: OrdersConfig):
    ...
    streams = BulkEngine(
        ...
        family="fulfillment_orders", search_filter=search_filter).prepare(now)
    ...
    for stream in STREAMS:
        ...
        manifest.update(shop_key=config.expected_shop_gid, stream=stream,
            ...
            provider_object_count=result["provider_object_count"], root_object_count=result["root_object_count"],
        ...
        metadata = {
            "raw_records": result["raw_record_count"], "extraction_id": config.extraction_id,
