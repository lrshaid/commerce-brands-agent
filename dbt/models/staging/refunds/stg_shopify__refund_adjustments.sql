...
-- Refund order adjustments, one row per adjustment.
with pages as (
...
select
    ...
    p.refund_gid,
    r.order_gid,
    json_value(n, '$.node.id') as adjustment_gid,
    cast(json_value(n, '$.node.amountSet.shopMoney.amount') as numeric) as amount,
    json_query(n, '$.node') as detail_payload
from pages p
cross join unnest(json_query_array(p.payload, '$.data.node.orderAdjustments.edges')) n with offset node_offset
...
where p.operation = 'orderAdjustments'
