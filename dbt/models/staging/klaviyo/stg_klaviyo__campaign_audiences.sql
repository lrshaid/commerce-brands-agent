...
-- exact captured pages.  The parent campaign always lives in the same page's
-- data[] (capture-validated), so children are keyed from their own page and no
-- cross-page join exists to fan out.
with pages as (
    select
...
select
    ...
    json_value(i, '$.attributes.audience_name') as audience_name,
    cast(json_value(i, '$.attributes.priority') as int64) as priority,
    cast(json_value(i, '$.attributes.has_non_draft_messages') as bool) as has_non_draft_messages,
    (select array_agg(json_value(l, '$')) from unnest(json_query_array(i, '$.attributes.included')) l) as included_list_ids,
    (select array_agg(json_value(l, '$')) from unnest(json_query_array(i, '$.attributes.excluded')) l) as excluded_list_ids,
    cast(json_value(i, '$.attributes.created_at') as timestamp) as created_at,
    cast(json_value(i, '$.attributes.updated_at') as timestamp) as updated_at,
