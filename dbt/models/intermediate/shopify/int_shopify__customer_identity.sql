...
-- guests (is_email_based = false). Multiple customer_gid sharing one email are
-- consolidated to a canonical customer with a deterministic tie-break (oldest
-- created_at, then highest amount_spent, then customer_gid). Marketing consent
-- is not captured by the customers projection, so no consent flag is asserted.
with customers as (
    select
        customer_gid,
        created_at,
        number_of_orders,
        amount_spent_amount,
        nullif(email, '') is not null as is_email_based,
        case
            when nullif(email, '') is null then customer_gid
...
ranked as (
    select
        c.*,
        row_number() over (
            partition by customer_identity_id
            order by
                created_at asc nulls last,
                amount_spent_amount desc nulls last,
                customer_gid asc
        ) as identity_rank,
        count(*) over (partition by customer_identity_id) as linked_customer_count
    from customers c
)
select
